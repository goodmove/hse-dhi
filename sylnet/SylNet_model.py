...
def get_weight_variable(name, shape=None, initializer=tf.keras.initializers.GlorotNormal()):
    if shape is None:
        return tf.compat.v1.get_variable(name)
    else:  
        return tf.compat.v1.get_variable(name, shape=shape, dtype=_FLOATX, initializer=initializer)
...
class CNET():
    ...
    def _input_layer(self, main_input):
        ...
        with tf.compat.v1.variable_scope('input_layer'):            
            ...
            Y = tf.compat.v1.layers.Dropout(rate=self.DRrate)(Y,training=self.isTrain)
    ...
    def _embed_cond(self, cond_input):
        ...
        with tf.compat.v1.variable_scope('embed_cond'):            
    ...
    def _conv_module(self, main_input, residual_input, module_idx, dilation, cond_input=None):
        ...
        with tf.compat.v1.variable_scope('conv_modules'):
            with tf.compat.v1.variable_scope('module{}'.format(module_idx)):
                                
                W = get_weight_variable('filter_gate_W',(fw, r, 2*r)) 
                b = get_bias_variable('filter_gate_b',(2*r))                                                          

                X = main_input
                Y = tf.compat.v1.nn.convolution(X, W, padding='SAME', dilation_rate=[dilation])
                #Y = self._causal_conv(X, W, fw)
                ...
                # filter and gate
                Y = tf.tanh(Y[:, :, :r])*tf.sigmoid(Y[:, :, r:])
                Y = tf.compat.v1.layers.Dropout(self.DRrate)(Y,training=self.isTrain)
    ...
    def _postproc_module(self, residual_module_outputs):
        ...
        with tf.compat.v1.variable_scope('postproc_module'):
            ...
            lstm_layer=tf.compat.v1.nn.rnn_cell.LSTMCell(num_units, reuse=tf.compat.v1.AUTO_REUSE, name='lstm_layer')
            outputsM,_=tf.compat.v1.nn.dynamic_rnn(lstm_layer,Y,sequence_length=self.seqLen,dtype="float32")
    ...
    def _last_layer(self, last_layer_ip):
        ...
        with tf.compat.v1.variable_scope('last_layer'):
            ...
            lstm_layer=tf.nn.rnn_cell.LSTMCell(num_units, reuse=tf.compat.v1.AUTO_REUSE, name='lstm_layer')
